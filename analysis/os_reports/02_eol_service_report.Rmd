...
_Table 2: Number of people who died at home and had at least one emergency admission in the last month_
```{r Table2
knitr::kable(read_csv(here::here("output", "os_reports", "eol_service", "emadm_month.csv")) %>%
 
 
admission_date.is_on_or_between(dod_ons - days(30), dod_ons)
).count_for_patient()
 
 
 
 mutate(start_date = as_date(cut(study_month, breaks = "1 months"))
         , end_date = rollback(start_date + months(3))
         , time_period = paste0(format(start_date, format = "%b-%y"), " to "
                                , format(end_date, format = "%b-%y"))) %>%
  group_by(time_period, start_date, end_date, pod_ons_new) %>%
  ...
  mutate(count = plyr::round_any(count, -1))
  pivot_wider(names_from = pod_ons_new, values_from = count) %>%
  col.names = c("Time period", "All", "Care home"
                                     , "Elsewhere/other", "Home", "Hospital", "Hospice")
```
